...
//    u.set_y(42.42);
    ...
    cout<<"s2 added\n";
    display(s2);
    Sphere* s3 =new Sphere(4, 6, 9, 1, RED);
    setSphere(s3);
    displaySphere();
    //test intersect
    Sphere s4(1, 1, 1, 2, GREEN);
    Ray r1(Vector3D(6, 1, 1), Vector3D(-1, 0, 0));
    pair<pair<Vector3D,Vector3D>,bool> p1 = intersect(r1, s4);
    assert(p1.second);
    cout<<"\nsphere : \n"<<s4<<"\nand ray : \n"<<r1<<"\nhas intersection : \n";
    display(p1.first.first);
    display(p1.first.second);

    Ray r2(Vector3D(6, 3, 1), Vector3D(-1, 0, 0));
    pair<pair<Vector3D,Vector3D>, bool> p2 = intersect(r2, s4);
    assert(p2.second);
    cout<<"\nsphere : \n"<<s4<<"\nand ray : \n"<<r2<<"\nhas intersection : \n";
    display(p2.first.first);
    display(p2.first.second);

    Ray r3(Vector3D(6, 4, 1), Vector3D(-1, 0, 0));
    pair<pair<Vector3D,Vector3D>, bool> p3 = intersect(r3, s4);
    assert(!p3.second);
    cout<<"\nsphere : \n"<<s4<<"\nand ray : \n"<<r3<<"\nhas no intersection.\n";
